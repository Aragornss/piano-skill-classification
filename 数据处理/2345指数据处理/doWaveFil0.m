%% doWaveFil0函数，数据滤波
function doWaveFil0(angle1,angle2,angle3,angle4)
%% 数据滤波
%平滑均值滤波-Lab10         https://blog.csdn.net/guomutian911/article/details/41576689
figure('NumberTitle', 'off', 'Name', '食指滤波曲线观察');%更好更有用
DL1=length(angle3);
...
%滤波——CMC
x_CMC=angle4;
b_CMC=filter2(fspecial('average',10),x_CMC)*10;%fspecial()函数为均值滤波算子；filter()函数求滑动平均
p04=plot(b_CMC(5:DL1-5),'R','LineWidth',1);
hold on;
...
set(gcf,'color','w');
h1=legend([p01, p02, p03,p04],'MCP','PIP','DIP','CMC');
