%% printAngRel函数，绘制小拇指各关节转角变化曲线和各关节转角之间的关系图像
function printAngRel3(CurA,CurB,CurC)
%绘制小拇指指各关节转角变化曲线
figure('NumberTitle', 'off', 'Name', '小拇指各关节转角曲线');
hold on;
plot(CurA);
plot(CurB);
plot(CurC);
...
%% 绘制各关节转角之间的关系曲线
figure('NumberTitle', 'off', 'Name', '小拇指各关节转角之间的关系');
subplot(2,2,1);
printType(CurA,CurB);
%axis([55,110, 10,70])
%title('MCP与PIP夹角关系')
